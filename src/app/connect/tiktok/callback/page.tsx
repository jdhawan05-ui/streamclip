...
function TikTokCallbackInner() {
  ...
  useEffect(() => {
    ...
    if (errParam) {
      setError("TikTok authorization was cancelled or denied.");
      setStatus("error");
      return;
    }
    ...
    fetch(`${API}/api/socials/tiktok/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`, {
      headers: token ? { Authorization: `Bearer ${token}` } : {},
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.ok) {
          setUsername(data.username || "TikTok");
          setStatus("success");
      ...
      .catch((e: unknown) => {
        setError((e as Error).message || "Failed to connect TikTok");
        setStatus("error");
      });
  }, [searchParams, router]);
    ...
    <div className="bg-[#0a0a0a] border border-white/8 rounded-2xl p-10 max-w-sm w-full text-center space-y-6">
      {status === "loading" && (
        <>
          ...
          <div>
            <h2 className="text-white font-semibold text-lg">Connecting TikTok</h2>
      ...
      {status === "success" && (
        <>
          ...
          <div>
            <h2 className="text-white font-semibold text-lg">TikTok Connected!</h2>
